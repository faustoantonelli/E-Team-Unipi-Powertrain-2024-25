...
// Helper function: Pedal position mapping
float pedal_map(float pps_in, float pps_max_threshold, float pps_min_threshold) {
    enum pedal_maps {
        LINEAR,
        EXP,
        LOG
    ...
    enum pedal_maps map_selector = LINEAR;

    float pps_out = 0;

    switch (map_selector) {
    case LINEAR: { // linear map
        pps_out = fmax(0, fmin((pps_in - pps_min_threshold) / (pps_max_threshold - pps_min_threshold) * pps_max_threshold, pps_max_threshold));
        break;
    }
...
// Helper function: Current control based on pedal position
float current_control(float pps_in, float pps_max_threshold, float pps_min_threshold, float i_max) {
...
float launch_control(
    ...
    float slip_smoothing = 1.0f;           // Smoothing factor for slip measurement
    float kp_launch = 1.0f; //200.0f;      // Proportional gain for fine slip control
    float max_current_step = 10.0f;        // Maximum allowed current adjustment factor

    // Launch control state variables
    static bool lc_active = false;         // Launch control active flag
    static float last_slip = 0.0f;         // Last slip value (for smoothing)
    static int debounce_counter = 0;       // Counter for debouncing launch control activation
    ...
    if (! lc_active) {
        ...
        // Change in case of rain (zio pera)
        ...
        // TO ADD MINIMUM SPEED TO AVOID BAD BEHAVIOUR (DONE, CHECK)
        ...
        delta_slip = (1.0f - slip_smoothing) * last_slip + slip_smoothing * delta_slip;
        last_slip = delta_slip;
        ...
        current = base_current - current_adjustment;
        ...
        // [TODO] REMEMBER TO LIMIT THE CURRENT IN ORDER TO NOT EXCEED NEITHER CURRENT NOR POWER LIMITS
        return current;
    }
}